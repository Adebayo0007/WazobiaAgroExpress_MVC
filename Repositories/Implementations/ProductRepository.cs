...
namespace Agro_Express.Repositories.Implementations
{
    public class ProductRepository : IProductRepository
    {
        ...
        public async Task DeleteProduct(Product product)
        {
         ...
         {
             var expiredProduct = await _applicationDbContext.Products
                .Where(p =>p.AvailabilityDateTo.Date.Day  <  DateTime.Now.Date.Day)
                .ToListAsync();
        ...
        public async Task<IEnumerable<Product>> GetAllFarmProductByLocationAsync(LocalGovernment buyerLocalGovernment, User user) =>
             await _applicationDbContext.Products.Where(p => p.ProductLocalGovernment == buyerLocalGovernment && p.ProductLocalGovernment == user.Address.LocalGovernment).ToListAsync();
        ...
        public  async Task<IEnumerable<Product>> SearchProductsByProductNameOrFarmerUserNameOrFarmerEmail(string searchInput, User user) =>
                ...
                .Where(p => p.FarmerEmail.ToLower() == searchInput.ToLower().Trim() && p.ProductLocalGovernment == user.Address.LocalGovernment || 
                            p.FarmerUserName.ToLower() == searchInput.ToLower().Trim() && p.ProductLocalGovernment == user.Address.LocalGovernment || p.ProductName.ToLower() == searchInput.ToLower().Trim() && p.ProductLocalGovernment == user.Address.LocalGovernment)
                .ToListAsync();
