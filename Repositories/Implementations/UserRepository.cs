...
namespace Agro_Express.Repositories.Implementations
{
    public class UserRepository : IUserRepository
    {
        ...
        public User GetByIdAsync(string userId) =>
             _applicationDbContext.Users.Include(a => a.Address).Include(a => a.Buyer).Include(a => a.Farmer).Include(a => a.Admin).SingleOrDefault(a => a.Id == userId);
        ...
        public async Task<IEnumerable<User>> SearchUserByEmailOrUsername(string searchInput) =>
            await _applicationDbContext.Users
            .Include(u =>u.Address)
            .Where(u => u.UserName.ToLower() == searchInput.ToLower().Trim() || 
                   u.Email.ToLower() == searchInput.ToLower().Trim())
            .ToListAsync();
